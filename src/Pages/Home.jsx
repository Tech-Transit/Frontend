...
const Home = () => {
    ...
    useEffect(() => {
        ...
        const coordinates = coordinatesData[0].coordinates;

        if (!mapInstanceRef.current) {
            mapInstanceRef.current = L.map(mapRef.current).setView([coordinates[0][0], coordinates[0][1]], 5);
        ...
        for (let i = 0; i < latLngs.length - 1; i++) {
            ...
            L.polyline([latLngs[i], latLngs[i + 1]], { color: 'red', dashArray: '4 4' }).addTo(mapInstanceRef.current);
            L.marker(midPoint, {
                icon: L.divIcon({
